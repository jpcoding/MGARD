...
public:
  QuantPred(T* quant_inds, int N, const size_t* dims, int start_level, int unpred, int quant_radius, int decompse = 1) {
    N = N; 
    num_elements = 1;
    for(int i=0; i<N; i++){
      num_elements *= dims[i];
  ...
  inline T  get_quant_compensate (size_t idx, size_t stride1, size_t stride2)
  {

     T A = aux_quant_inds[idx - stride1 - stride2];
     T B = aux_quant_inds[idx - stride1];
     T C = aux_quant_inds[idx - stride2];
  ...
  inline T  get_quant_compensate_recover (size_t idx, size_t stride1, size_t stride2)
  {

     T A = orig_quant_inds[idx - stride1 - stride2];
     T B = orig_quant_inds[idx - stride1];
     T C = orig_quant_inds[idx - stride2];
     if(A == unpred || B == unpred || C == unpred){
      return 0;
     }
     if( B > quant_radius && C > quant_radius){ 
...
private:
  ...
  int start_level = -1;
  std::vector<T> aux_quant_inds; 
  T* orig_quant_inds;
  int unpred = 0;
  int quant_radius = 1<<15;
};
